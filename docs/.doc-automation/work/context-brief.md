...
## 第三轮：介绍页与操作页改写（2026-09-20）

用户请求原文：「在现有操作手册里面增加两个页面：1. 介绍页（类似首页）：介绍我们这个功能
解决了哪些痛点，让用户知道为什么要用这个插件。2. 操作页：教用户怎么快速使用。这两个重点
介绍页很重要，关系到用户是否能用你这个项目。」以 `/eli5` 调用，即要求「图多字少」的表达。

盘点结论：这两页**已经存在**，不新增。`index.md` 已承担介绍页（含「它解决的问题」与阅读
顺序），`quickstart.md` 已承担操作页（安装 → 第一次预览）。再建两页讲同一件事，等于制造
第二套真源，且入口仍是 `index.md`，新页无人走到。

| 必问项 | 状态 | 内容 / 出处 |
|---|---|---|
| 业务目标与成功标准 | ✅ 用户已答 | 目标：让没装过的人看完知道为什么要用，并且能照着装起来。成功标准：用户原话「关系到用户是否能用你这个项目」 |
| 输出文档类型与受众 | ✅ 用户已答 | 受众=**还没安装、正在判断要不要用**的开发者（用户在选项中明确选定）。类型不变：`index` 仍是 explanation，`quickstart` 仍是 tutorial |
| 与既有文档的关系 | ✅ 用户已答 | 改写现有两页（用户在三选一中选定），不新增文件，入口不变，中英双份同步改 |
| 范围边界 | ✅ 用户已答 | 只动这两页；其余四页与 README 本轮不动 |

本轮不改变事实，只改变**呈现密度**：命令、参数、`预期` 措辞一律沿用上一轮从实录
（E008、E009）逐字取回的原文，不重新表述，避免文字漂移出实际输出。
