...
## Batch-5：介绍页与操作页改写（2026-09-20，第三轮）

触发：用户以 `/eli5` 要求「图多字少」地讲清痛点与上手路径，受众定为**还没安装的人**。

| 项 | 内容 |
|----|------|
| 页数预算 | **不增页**。改写 2 页 × 2 语言 = 4 个文件；手册仍是 6 页每语言 |
| 最小集 | `index.md`（痛点对照图 + 能力表 + 真实截图 + 阅读顺序）· `quickstart.md`（五步总览图 + 两段流程） |
| 扩展集 | 新建 `why.md` / `start-here.md` —— 用户已否决，**不写**（会与现有两页并存成两套真源） |
| 依据 | 功能地图未变，页数不变；改的是密度契约那一侧：`index` 正文 ≤500 字、`quickstart` ≤800 字，单段 ≤3 行，痛点改用图承载 |

### 验收标准

- `index.md`：第一屏是痛点对照图而非文字；保留「链接即密码」与 PowerShell `mp.cmd`
  两条安全提示（删掉它们等于让新手把预览链接当普通链接转发）；保留范围边界
- `quickstart.md`：首屏一张五步总览图；两段流程各自 ≤7 步、各有 `起点：`、末步 `- 预期：`；
  `预期` 字符串与 E008 / E009 实录逐字一致
- 中英两份改动一一对应，英文页用 `## Prerequisites` / `Start:` / `- Expected:` /
  `## Verify` / `## If it fails`
- `quality-check.mjs` 退出码 0
