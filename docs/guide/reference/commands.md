...
| 命令 | 作用 | 最常用的参数 |
|---|---|---|
| `mp start` | 把本地端口开成临时的带口令公网链接 | `--port` `--serve` `--ttl` `--dev` |
...
| 参数 | 默认 | 说明 |
|---|---|---|
| `--port <n>` | 5173 | 要暴露的本地端口，即你的应用已经监听的那个；与 `--serve` 互斥 |
| `--serve <path>` | 无 | 由 mp 自己端这个文件或目录，不再代理你另起的服务器。mp 挑一个空端口，服务器跑在预览进程里，预览到期它一起没了。给目录是整个目录，给单个文件就只有这一个文件（邻居不会被顺带挂上去）
